...
class IpAnalyst
{
     ...
     * @return mixed|string
     */
    public static function getRegionDetail($ip)
    {
        ...
        try {
            ...
            if ($node > 0) {
                ...
                $location[] = '';
                $region = self::findRegionCode($location);
                $location[] = $region;
     ...
     * @return string
     */
    private static function findRegionCode($arr)
    {
        $province = $arr[1];
        $city = $arr[2];
        $region = self::regionList();
        $code = "";
        if (!isset($region[$province])) {
            return $code;
        }
        $code = $region[$province]["code"];
        if (!empty($city)) {
            foreach ($region[$province]["cities"] as $key => $loc) {
     ...
     * @return mixed|string
     */
    public static function getRegionCode($ip)
    {
        $findResult=self::getRegionDetail($ip);
        return $findResult[4];
    }
     ...
     * @return mixed|string
     */
    public static function getRegionInfo($ip)
    {
        $findResult=self::getRegionDetail($ip);
        unset($findResult[4]);
        array_unique($findResult);
        return join(' ',$findResult);
    }
